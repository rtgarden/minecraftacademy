...
GROUNDLIST = [1,2,3,4,7,12,13,24,48,49,82]
...
if len(GROUNDLIST) < 1:
    print("There is nothing in GROUNDLIST, you may encounter problems.")
if DEPTHSCALE < 0:
    print("DEPTHSCALE is negative, setting to zero")
    DEPTHSCALE = 0
...
def find_surface(x,y,z,search,mclevel,invert=False):
    ...
    while True:
        ...
        void_check = info['B'] not in search
        # flip the truth value if invert is True
        if invert: truth_check = not truth_check
...
class Square(object):
    ...
    def __init__(self,mclevel,x,y,z):
        self.x = x
        self.y = find_surface(x,y,z,GROUNDLIST,mclevel)
        self.z = z
    ...
    def grow(self):
        ...
        if new_weight >= 0 and self.weight < 0:
            flag = True
        else:
            flag = False
...
class FlyingMountain(object):
    ...
    def create(self):
        get_block = self.save_file.block
        ...
        # Default "air" block
        air_block_data = {'B':0,'D':0,'S':0,'L':2}
        for square in self.interior.values():
            y = square.y
            ...
            this_depth = int(DEPTHSCALE * (depth_scale * square.weight + 
            ...
            old_height = self.save_file.retrieve_heightmap(x,z)
